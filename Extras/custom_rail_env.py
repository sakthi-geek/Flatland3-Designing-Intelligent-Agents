...
class RailEnv(Environment):
    ...
    def _get_spaces(self):
        ...
        action_spaces = {agent: gym.spaces.Discrete(5) for agent in range(self.number_of_agents)}

        if self.obs_type == 'tree':
            n_features_per_node = self.obs_builder.observation_dim
            print(n_features_per_node)
            n_nodes = sum([np.power(4, i) for i in range(self.max_tree_depth + 1)])
            print(n_nodes)
            state_size = n_features_per_node * n_nodes
            print(state_size)
            obs_space = gym.spaces.Box(low=-np.inf, high=np.inf, shape=(state_size,))
            observation_spaces = {agent: obs_space for agent in range(self.number_of_agents)}
